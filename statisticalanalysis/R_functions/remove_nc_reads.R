remove_nc_reads <- function(phyloseq, batch_id, NC_name){ 
  ...
  ### Arguments:
    ...
    # batch_id: character vector with the name of the batch ID. Assumes it exists as a variable of "PCRbatch" in the sample table
    # NC_name: character vector with the name of the NC sample.
    
  ### Function:
    # subset by Run
    batch_id <<- batch_id # assign batch_id to the global env
    batch <- subset_samples(phyloseq, PCRbatch == batch_id)
    print("input data")
    print(batch)
